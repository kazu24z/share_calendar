  ...
  if($_SERVER['REQUEST_METHOD']!='POST'){
      ...
      if($end_at ==""){
        $err['end_at'] = '終了日時を選択してください。';
          ...
          $err['end_at'] = '終了日時の形式が不正です。';
           ...
           //チェック条件式
           if($min_time > $end_at){
             $err['end_at'] = '開始日時の値が不正です。';
           }

       }

       if(@$err['start_at']==''&&@$err['end_at']==''){
    ...
    if(empty(@$err)){
      ...
      //データベース（userテーブル）に入力データを新規登録する。
        $sql ="INSERT INTO item (user_id,start_at,end_at,item_text,label_color,created_at,updated_at) VALUES (:user_id,:start_at,:end_at,:item_text,:label_color,now(),now())";
...
<html lang="ja">
  <head>
    <meta charset="utf-8">
    <title>予定の追加 | <?php echo SERVICE_NAME; ?></title>
    ...
    <div class="container">
      <div class="row">
        <div class="col-md-6 offset-md-3">
          <h3  class="text-center mt-3 mb-3">予定の追加</h3>
          ...
          <!--入力フォーム-->
            ...
            <div class="mb-3">
              ...
              <input type="datetime-local" placeholder="YYYY-MM-DDThh:mm" pattern="^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])T([01][0-9]|2[0-3]):[0-5][0-9]" id="start_at" name="start_at" value="<?php if(@$start_at !=''){ echo substr_replace(h(@$start_at),'T','10',1);}?>" class="form-control <?php if(@$err['start_at'] || @$err['time_wrap'] != ''){echo 'is-invalid';} ?>" min="1960-01-01T00:00" max="9999-12-31T23:59" >
                <!--エラー表示-->
            ...
            <div class="mb-3">
              ...
              <input type="datetime-local" placeholder="YYYY-MM-DDThh:mm" pattern="^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])T([01][0-9]|2[0-3]):[0-5][0-9]" id="end_at" name="end_at" value="<?php if(@$end_at != ''){ echo substr_replace(h(@$end_at),'T','10',1);}?>" class="form-control <?php if(@$err['end_at'] || @$err['time_wrap'] != ''){echo 'is-invalid';} ?>" min="1960-01-01T00:00" max="9999-12-31T23:59" >
                  <!--エラー表示-->
